...
class Zoom extends Module {
  ...
  zoomIn (event) {
    ...
    // Find the free range of screen for insert result globalThis.
  ...
  destroy () {
    this.zoomOut();
    this.sliderInstance.boardWrapper.removeEventListener('mouseover', this._zoomIn);
    this.sliderInstance.boardWrapper.removeEventListener('mouseleave', this._zoomOut);
    this.lens.remove();
  ...
  sizingElements () {
    ...
    top = sliderRect.top + globalThis.scrollY;
    left = sliderRect.left + sliderRect.width + globalThis.scrollX + padding;
    ...
    if (spaceHeightAtTopOfSliderContainer > spaceWidthAtRightOfSliderContainer) {
      ...
      top = sliderRect.top + globalThis.scrollY - height - padding;
      left = sliderRect.left + globalThis.scrollX;
    }
    ...
    if (spaceHeightAtBottomOfSliderContainer > spaceHeightAtTopOfSliderContainer) {
      ...
      width = document.documentElement.clientWidth - padding - sliderRect.left;
      top = sliderRect.bottom + globalThis.scrollY + padding;
      left = sliderRect.left + globalThis.scrollX;
    }
  ...
  getRenderedSize () {
    ...
    if (naturalRatio > visibleRatio) {
      size.width = globalThis.Math.min(this.activeImage.width, this.activeImage.naturalWidth);
      size.height = globalThis.Math.min(this.activeImage.width, this.activeImage.naturalWidth) / naturalRatio;
    } else {
      size.width = globalThis.Math.min(this.activeImage.height, this.activeImage.naturalHeight) * naturalRatio;
      size.height = globalThis.Math.min(this.activeImage.height, this.activeImage.naturalHeight);
    }
  ...
  getCursorPositionRelativeToBoardWrapper (e) {
    ...
    const x = (e.pageX - a.left) - globalThis.scrollX;
    const y = (e.pageY - a.top) - globalThis.scrollY;
