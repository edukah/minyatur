...
class Img extends Module {
  ...
  animateToWidth (targetWidth, event, duration = 300) {
    ...
    const startTime = (typeof performance !== 'undefined' && globalThis.performance.now) ? globalThis.performance.now() : Date.now();
    this._preventPointerMove = true;

    const animate = (nowRaw) => {
      const now = (typeof performance !== 'undefined' && globalThis.performance.now) ? nowRaw : Date.now();
  ...
  show () {
    ...
    this.imageWrapper.classList.remove('is-hidden');
  }

  hide () {
    this.imageWrapper.classList.add('is-hidden');
  ...
  destroy () {
    if (this.imageElem) {
      this.imageElem.removeEventListener('pointerdown', this._pointerDownHandler);
      this.imageElem.removeEventListener('pointermove', this._pointerMoveHandler);
      this.imageElem.removeEventListener('touchstart', this._doubleTapHandler);
    }

    if (this.imageDiv) {
      this.imageDiv.removeEventListener('wheel', this._wheelZoom);
      this.imageDiv.removeEventListener('touchstart', this._handlePinchStart);
      this.imageDiv.removeEventListener('touchmove', this._imagePinchMove);
      this.imageDiv.removeEventListener('touchend', this._handlePinchEnd);
    }
  ...
  getZoomMaxWidth () {
    return Math.min(this.imageElem.naturalWidth * this.getZoomMaxScale(), this.imageElem.width * this.getZoomMaxScale(), globalThis.innerWidth * 3);
  }
  ...
  zoomIn (targetWidthWithoutPx, event = null) {
    ...
    this.imageElem.classList.remove('is-zoom-out');
    this.imageElem.classList.add('is-zoom-in');
  }

  zoomOut () {
    this.imageElemWidth = null;

    this.imageElem.classList.remove('is-zoom-in');
    this.imageElem.classList.add('is-zoom-out');
  }
  ...
  pointerClickHandler (event) {
    ...
    if (this.imageElem.classList.contains('is-zoom-in')) {
      this.zoomOut();
  ...
  wheelZoom (event) {
    ...
    // Zoom limits
    const minWidth = Math.min(globalThis.innerWidth, this.imageElem.naturalWidth);
  ...
  doubleTapHandler (event) {
    ...
    if (!this.tapped) {
      ...
      this.tapTimeoutId = globalThis.setTimeout(() => {
        this.tapped = null;
      }, 300); 
    ...
    const deltaX = event.touches[0].pageX - this.tapped.x;
    const deltaY = event.touches[0].pageY - this.tapped.y;
